...
async def add_pantry_item(item: PantryItemCreate, user_id: int = Depends(get_current_user)):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO pantry_items (user_id, name, calories) VALUES (?, ?, ?)",
            (user_id, item.name, item.calories)
        )
        await db.commit()
        item_id = cursor.lastrowid
    return PantryItemOut(id=item_id, user_id=user_id, **item.dict())
...
async def list_pantry_items(
    ...
    query = "SELECT id, name, calories FROM pantry_items WHERE user_id = ?"
    params = [user_id]
    if search:
        query += " AND name LIKE ?"
        params.append(f"%{search}%")
    # For category, you would need a category column in the DB. Add if needed.
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(query, params)
        rows = await cursor.fetchall()
    return [PantryItemOut(id=row[0], user_id=user_id, name=row[1], calories=row[2]) for row in rows]
